...
namespace
{
	...
	Callback sPreviousCallback = nullptr;
}
...
LRESULT CALLBACK Input::InputSystemMessageHandler(HWND window, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	return CallWindowProc((WNDPROC)sPreviousCallback, window, message, wParam, lParam);
}
...
void InputSystem::Initialize(HWND window)
{
	...
	mWindow = window;
	sPreviousCallback = (Callback)GetWindowLongPtr(window, GWLP_WNDPROC);
	SetWindowLongPtr(window, GWLP_WNDPROC, (LONG_PTR)InputSystemMessageHandler);
...
void InputSystem::Terminate()
{
	...
	SetWindowLongPtr(mWindow, GWLP_WNDPROC, (LONG_PTR)sPreviousCallback);
	mWindow = nullptr;
