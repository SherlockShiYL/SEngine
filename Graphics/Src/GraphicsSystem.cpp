...
void GraphicsSystem::Initialize(HWND window, bool fullscreen)
{
	...
	hr = mD3DDevice->CreateBlendState(&blendStateDescription, &mAlphaDisableBlendState);
	ASSERT(SUCCEEDED(hr), "[GraphicsSystem] Failed to create blend state.");

	blendStateDescription.RenderTarget[0].BlendEnable = TRUE;

	// Create the blend state using the description
	hr = mD3DDevice->CreateBlendState(&blendStateDescription, &mAlphaEnableBlendState);
	ASSERT(SUCCEEDED(hr), "[GraphicsSystem] Failed to create blend state.");
...
{
	ASSERT(mSwapChain != nullptr, "[GraphicsSystem] Failed to get swap chain buffer width.");
	return static_cast<float>(mSwapChainDesc.BufferDesc.Width) / static_cast<float>(mSwapChainDesc.BufferDesc.Height);
}